...
//   0.70  hit: sideways water fan and falling drops on the target; burning scenario, the flames go out in a puff of steam
//   0.70+ soaked: the pawn is tinted and drips; falling splash drops grow a puddle from 0.88 s
...
// and a rear third that separates into slower water parcels after the nozzle closes.
// Trail parcels spread and fall from their emission times; tangent-based width holds in all facings.
// Contact is an alpha-blended sideways fan, ballistic drops and wet landing patches.
// Steam uses the Six Paths Puff texture.
import { Color, Mathf } from '../js/engine.js';
import { P, Y, sprite, soft } from './lib/six-paths-impact.js';
import { drawWaterGun, frame, figure, flames, steam, drips, shotImpact, stream, streamTrail, bump, Water, Lead, Raise, Lower, Tail } from './lib/water-gun.js';
...
export default {
  ...
  draw(s, p, { origin: centre, scene }) {
    ...
    if (fired && u0 < 1) {
      stream('water gun jet', g.muzzleS, target, u0, u1, p.width, s, Y + .02, .05, 0, smooth((s - t.fire - Jet) / .06));
      const mz = 1 - clamp((s - t.fire) / .12);
      sprite(g.muzzleS, .26, .22, Water.withAlpha(mz * .6), soft, Y + .03);   // wet muzzle spray
    }
    streamTrail('water gun wake', g.muzzleS, target, p.width, s - t.fire, p.flight, Jet);
    shotImpact('water gun hit', o, hitAge, f, strength);
  },
};
