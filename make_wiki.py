...
# Behaviour that lives in C# rather than in the defs. Keyed by defName; printed under "Details".
NOTES = {
    "AG_FumaShuriken": [
        "Choose a landing cell with the throw button while drafted. There is no accuracy roll; "
        "it flies along the highlighted line.",
        "Each pawn is hit once. Allies, downed pawns, animals and mechanoids can all be hit.",
        "Walls and closed doors stop it; low cover and open doors let it through. Personal shields "
        "can absorb the damage; area projectile shields stop the flight.",
        "Throwing leaves the pawn unarmed. The same weapon lands with its quality and condition "
        "intact. Use Equip to pick it up, or pull it closer with the retrieval hook first. Giving "
        "it to another pawn does not reset the throw cooldown.",
        "With Melee Animation the throw plays an unfolding animation. With Combat Extended it gets "
...
for tag, name in mod_keys:
    ...
    if tag == "GeneDef" and el.findall("abilities/li"):
    ...
    elif tag == "ThingDef" and el.find("comps/li[@Class='RimArt.CompProperties_Fuma']") is not None:
        # The throw is a C# command, not a verb, so its numbers come from the source constants.
        rules = open("Source/RimArt/Fuma/FumaRules.cs").read()
        ...
        while not hits or hits[-1] > 8:
            hits.append(max(8, round(30 * 0.8 ** len(hits))))
        proj = get("ThingDef", "AG_FumaProjectile")
        # Projectile speed is in hundredths of a cell per tick.
        speed = number(round(float(text_of(proj, "projectile/speed")) / 100 * 60, 1))
        details = [melee,
                   "Throw damage: " + "/".join(map(str, hits)) + " to the 1st, 2nd, 3rd... pawn in the path, "
                   "then 8 to every pawn after that. " + number(float(text_of(proj, "projectile/armorPenetrationBase")) * 100)
                   + "% armor penetration.",
                   "Flies at " + speed + " cells per second."] + NOTES.get(name, [])
        kits.append({"type": "Weapon", "name": text_of(el, "label"), "desc": text_of(el, "description"),
...
for k in kits:
    ...
    if k.get("details"):
        out += ["**Details**", ""] + ["- " + line for line in k["details"]] + [""]
